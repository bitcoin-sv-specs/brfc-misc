...
## Calculating Tx Fee with Different Fee Types:
...
Bytes which are considered as `data` and therefore qualify for the `data` fee type rate are bytes of scripts that start with `6a` or `006a` (`OP_RETURN` or `OP_0 OP_RETURN`). This implies that only the following bytes are charged using the `data` fee rate:
```
